Template.ListData.helpers({
  ...
  userList: function () {
    return _.map(RadGrad.user.find({}, {sort: {lastName: 1}}).fetch(), function(user) {
      ...
      return { picture: user.picture,
        description: name + roles + '<br>'  + email + degreeGoals + aboutMe + tags + degreePlan + gradSemester};
    })
  },
  ...
  courseList: function() {
    return _.map(RadGrad.course.find({}, {sort: {number: 1}}).fetch(), function(course) {
      ...
      let credithrs = `<br>*Credit Hrs:* ${course.credithrs}`;
      return {description: number + name + slugName + tagNames + credithrs + description}
    })
  },
  ...
  courseInstanceList: function() {
    return _.map(RadGrad.courseinstance.find().fetch(), function(courseInstance) {
      ...
      let credithrs = courseInstance.credithrs;
      return {semester: semester, course: course, verified: verified, grade: grade, credithrs: credithrs ,student: student, note: note}
    })
  },
  ...
  degreePlanList: function() {
    return _.map(RadGrad.degreeplan.find().fetch(), function(degreePlan) {
      ...
      let student = `**Student:** ${RadGrad.user.findOne(degreePlan.studentID).lastName}<br>`;
      
      return {description: student + courseInstanceString + opportunityInstanceString + workInstanceString}
    })
  },
  ...
  opportunityList: function () {
    return _.map(RadGrad.opportunity.find({}, {sort: {name: 1}}).fetch(), function(opportunity) {
      ...
      let slug = `<br>**Slug:** ${RadGrad.slug.findBySlug(opportunity.slugID).name}, `;
      let opportunityType = `<br>**Type:** ${RadGrad.opportunitytype.findOne(opportunity.opportunityTypeID).name}, `;
      let sponsor = `<br>**Sponsor:** ${RadGrad.user.findOne(opportunity.sponsorID).lastName}`;
  ...
  opportunityTypeList: function () {
    return _.map(RadGrad.opportunitytype.find({}, {sort: {name: 1}}).fetch(), function(opportunityType) {
   ...
   */
  slugList: function () {
    return _.map(RadGrad.slug.find({}, {sort: {name: 1}}).fetch(), function(slug) {
      let name = `${slug.name}`;
  ...
  tagTypeList: function () {
    return _.map(RadGrad.tagtype.find({}, {sort: {name: 1}}).fetch(), function(tagType) {
